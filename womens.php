...
// If not logged in, store product in session and redirect to login
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_to_cart'])) {
    if (!$user_id) {
        $_SESSION['pending_cart'] = $_POST; // Save item temporarily
        header("Location: login.php");
        exit;
    }

    // If logged in, insert into cart table
...
// After login, insert pending cart item
if ($user_id && isset($_SESSION['pending_cart'])) {
    $post = $_SESSION['pending_cart'];
    $stmt = $conn->prepare("INSERT INTO cart (user_id, product_id, color, size, price) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("iisss", $user_id, $post['product_id'], $post['color'], $post['size'], $post['price']);
    $stmt->execute();
    $stmt->close();
    unset($_SESSION['pending_cart']);
}

$result = $conn->query("SELECT * FROM women_products");
?>
...
<head>
    ...
    <style>
        * {box-sizing: border-box; margin: 0; padding: 0; font-family: Arial;}
        ...
        .btn {background-color:rgb(239, 11, 239); color: white; border: none; cursor: pointer;}
        .btn:hover {background-color:rgb(201, 15, 210);}
